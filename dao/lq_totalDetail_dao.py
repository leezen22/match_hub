...
class LqTotalDetailDao(object):
    @staticmethod
    def insert(companyId, data, keys=None, isDict=True, isPre=True):
        if isPre:
            tableName = "lq_asianOdds_pre_{0}".format(companyId)
        else:
    ...
    def insertMany(companyId, data, keys=None, isDict=True, isPre=True):
        if isPre:
            tableName = "lq_totalodds_pre_{0}".format(companyId)
        else:
            tableName = "lq_totalodds_gun_{0}".format(companyId)
