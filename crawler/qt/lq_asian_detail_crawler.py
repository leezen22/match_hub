...
class AsianDetailCrawler(object):
    ...
    def qt_mobile_url(self):
        return "{0}/{1}/{2}.htm".format(scrawler_config.qt_lq_mobile_asianDetail_url,
                                        self.companyId, self.scheduleId)

    @property
    def qt_mobile_keys(self):
        keys = ['matchId', 'scheduleId', 'companyId', 'oddsId', 'homeOdds', 'goal', 'awayOdds', 'isBet',
                'oddsType', 'kind', 'modifyTime']
        return keys
                    ...
                    if counts > 0:
                        ...
                        for i in range(0, counts):
                            ...
                            detail = [self.matchId, self.scheduleId, self.companyId, self.oddsId,
                                      homeOdds, goal, awayOdds, 1, oddsType, 6, modifyTime
                                      ]
...
#     str_p = '2019-10-14 11:30'
#     matchTime2 = datetime.strptime(str_p, '%Y-%m-%d %H:%M')
#     detailCrawler = AsianDetailCrawler(199152, 8, 366987, 89991, -1, matchTime2, 0)
#     data = detailCrawler.qt_mobile_pre_detail()
#     print(data)
#     str_p = '2020-02-08 03:05'
#     matchTime2 = datetime.strptime(str_p, '%Y-%m-%d %H:%M')
#     detailCrawler = AsianDetailCrawler(196002, 1, 362944, 646672, -1, matchTime2, 0)
#     data = detailCrawler.qt_mobile_pre_detail()
#     print(data)
