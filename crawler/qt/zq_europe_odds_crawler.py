    ...
    def qt_web_get(self, hasDetail=False):
        ...
        if webResponse[0] == 1 and content != '':
            try:
                ...
                print("zq europe web odds parse failed: scheduleId={0}, error={1}".format(self.scheduleId, e))
                if oddsData['odds']:
                    oddsData['state'] = 1
        # 返回亚指开盘公司初盘和终盘盘口
        return oddsData
    ...
    def qt_mobile_get(self):
        ...
        if webResponse[0] == 1 and webResponse[1] != '':
            ...
            try:
                ...
                print("zq europe mobile odds parse failed: scheduleId={0}, error={1}".format(self.scheduleId, e))
                if oddsData['odds']:
                    oddsData['state'] = 1
                # 获取页面成功
            else:
...
def get_byJS(jsContent, matchId, scheduleId=None, hasDetail=True):
    ...
    try:
        ...
        print("zq europe odds js parse failed: scheduleId={0}, matchId={1}, error={2}".format(
            scheduleId, matchId, e))
        if company_odds:
            oddsData['odds'] = company_odds
            oddsData['state'] = 1
    return oddsData
